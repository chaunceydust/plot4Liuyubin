...
stat.test.data3 <- rawdata %>% 
  ...
  rstatix::t_test(Result ~ Time, paired = TRUE) %>% 
  ungroup() %>%
  group_by(Group, INDEX) %>%
  mutate(
    batch = row_number(),
...
fig1 <- ggplot(
  ...
  geom_text(
    data = plot.flag.data,
    aes(x = Time, y = y_max0*1.03, label = flag1),
  ...
  geom_text(
    data = plot.flag.data,
    aes(x = Time, y = y_max0*1.09, label = flag2),
...
fig2.1 <- ggplot(
  ...
  geom_text(
    data = plot.flag.data,
    aes(x = Time, y = y_max0*1.01, label = flag1),
    color = "#3019FF", #("#EE2C2C") # "#FFFFFF", "#FFFFFF")
    # face = "bold", 
    size = 4.8
  ) +
...
ggsave("Fig2-1-v3.pdf",
       width = 9, height = 6)
ggsave("Fig2-1-v3.tiff",
       width = 9, height = 6)
ggsave("Fig2-1-v3.png",
       width = 9, height = 6)
...
fig2.2 <- ggplot(
  ...
  geom_text(
    data = plot.flag.data,
    aes(x = Time, y = y_max0*1.05, label = flag1),
    color = "#3019FF", #("#EE2C2C") # "#FFFFFF", "#FFFFFF")
    # face = "bold", 
    size = 4.8
  ) +
  geom_text(
    data = plot.flag.data,
    aes(x = Time, y = y_max0*1, label = flag2),
    color = "#947F00", #("#EE2C2C") # "#FFFFFF", "#FFFFFF")
    # face = "bold", 
    size = 4.8
  ) +
...
ggsave("Fig2-2-v3.pdf",
       width = 9, height = 6)
ggsave("Fig2-2-v3.tiff",
       width = 9, height = 6)
ggsave("Fig2-2-v3.png",
       width = 9, height = 6)
...
fig3 <- ggplot(
  ...
  #   aes(Time, MEAN, color = Group, group = Group),
  #   size = size_of_line,
  #   position = position_nudge(x = -mynudge, y = 0)
  # ) +
  geom_smooth(
    data = filter(plot.summ.data, Group == "low dose group"),
    aes(Time, MEAN, color = Group, group = Group),
